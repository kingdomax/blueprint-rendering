Shader "Unlit/EdgeMapping"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			// An edge enhancement technique samples the neighboring pixels for each pixel, and calculates the degree of discontinuity in depth and normal.
			fixed4 frag (v2f i) : SV_Target
			{ 
				float4  p = tex2D(_MainTex, i.uv);
			    
				if (abs(dot(p, (0.25).xxxx) - 1.) < 0.001) discard; // nothing to render?
				...
				float normDis   = 1. - 0.5*(dot(A.xyz, H.xyz) + dot(C.xyz, F.xyz)); // crease edge = normal map discontinue
				float depthDis  = (1. - 0.5*abs(A.w - H.w))*(1. - 0.5*abs(A.w - H.w)); // sihoullete edge & border edge = depth discontinue
